...
Partial Class Exempt
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            ...
            Select Case globalMessage
                Case "1"
                    lblMessage.Text = "Exempt Has Been Added."
                    lblMessage.ForeColor = Drawing.Color.Green
                    lblMessage.Visible = True
                Case "2"
                    lblMessage.Text = "Exempt Has Been Deleted."
                    lblMessage.ForeColor = Drawing.Color.Green
                    lblMessage.Visible = True
                Case "3"
                    lblMessage.Text = "Exempt Has Been Updated."
                    lblMessage.ForeColor = Drawing.Color.Green
                    lblMessage.Visible = True
                Case Else
    ...
    Sub PopulateDataGrid()
        ...
        '-------------------------------------------------------------
        ExemptDataGrid.CurrentPageIndex = 0
        getExempt("[IncidentType].[IncidentType] ASC", "", "")
        ExemptDataGrid.AllowSorting = True

    End Sub

    Sub getExempt(ByVal sSortStr As String, ByVal sSearchBy As String, ByVal sSearchText As String)

        'connect and build the datagrid.
        objConn.ConnectionString = ConfigurationManager.ConnectionStrings("dbConnectionString").ConnectionString
        DBConStringHelper.PrepareConnection(objConn) 'open the connection

        objCmd = New SqlCommand("spFilterExempt", objConn)
        ...
        objDataGridFunctions.CalcDataGridCounts(objDS, ExemptDataGrid, "")
        'Associate the data grid with the data
        ExemptDataGrid.DataSource = objDS.Tables(0).DefaultView
        ExemptDataGrid.DataBind()

        objDataGridFunctions.Highlightrows(ExemptDataGrid, "", "", "")

    End Sub

    Sub SortExempt(ByVal sender As Object, ByVal e As DataGridSortCommandEventArgs)
        ...
        For x = 0 To ExemptDataGrid.Columns.Count - 1
            TempHeaderHolder = ExemptDataGrid.Columns(x).HeaderText
            FindImg = InStr(TempHeaderHolder, "<img")
            If FindImg <> 0 Then
                ExemptDataGrid.Columns(x).HeaderText = Left(TempHeaderHolder, FindImg - 1)
            End If
        Next
        '--------------------------------------------------------------------------------------------------------
        ...
        ExemptDataGrid.Columns(iIndex).SortExpression = NewSortExpr
        'alter the column's header image
        ExemptDataGrid.Columns(iIndex).HeaderText = NewHeaderText & NewHeaderImg

        '--------------------------------------------------------------------------------------------------------
        ' Sort the data in new order
        'searches by provided
        getExempt(NewSortExpr, ddlSearchBy.SelectedItem.Value, HelpFunction.Convertdbnulls(txtSearch.Text))

    End Sub

    Sub ExemptDataGrid_PageIndexChanged(ByVal source As Object, ByVal e As DataGridPageChangedEventArgs)

        '---------------------------------------------------------------------------------------------------------
        '   This sub is called on the next and previous clicks for the recordset, it cycles to the next 20 records
        '---------------------------------------------------------------------------------------------------------
        ExemptDataGrid.CurrentPageIndex = e.NewPageIndex
        ExemptDataGrid.DataBind()
        ...
        For x = 0 To ExemptDataGrid.Columns.Count - 1
            FindImg = InStr(ExemptDataGrid.Columns(x).HeaderText, "<img") 'find the column with the <img tag
            If FindImg <> 0 Then
                TempSortHolder = ExemptDataGrid.Columns(x).SortExpression
        ...
        If strSort = "" Then strSort = "Exempt.[ExemptID] ASC"


        getExempt("", ddlSearchBy.SelectedItem.Value, HelpFunction.Convertdbnulls(txtSearch.Text))
    ...
    Protected Sub ExemptDataGrid_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles ExemptDataGrid.ItemDataBound
    ...
    Private Sub DeleteExempt()
        ...
        Try
            ...
            AuditAction = "Deleted " & MrDataGrabber.GrabOneStringColumnByPrimaryKey("IncidentType", "IncidentType", "IncidentTypeID", localIncidentTypeID) & " from Exempts"

            objConn.ConnectionString = ConfigurationManager.ConnectionStrings("dbConnectionString").ConnectionString

            '// Enter the email and password to query/command object.
            objCmd = New SqlCommand("spDeleteExemptByExemptID", objConn)
            objCmd.CommandType = CommandType.StoredProcedure
            objCmd.Parameters.AddWithValue("@ExemptID", Request("ExemptID"))
            ...
            Response.Redirect("Exempt.aspx?Message=2")

        Catch ex As Exception

            DBConStringHelper.FinalizeConnection(objConn)
            lblMessage.Text = "You may not delete this Exempt due to the fact it is tied to related imported information. You must first delete all related imported information first, and then you may delete the Exempt."
            lblMessage.Visible = True
