...
Public Class HelpFunctions
    ...
    Public Function Convertdbnulls(ByVal x) As String
    ...
    Public Function ConvertdbnullsInt(ByVal x) As Integer
    ...
    Public Function ConvertdbnullsDbl(ByVal x) As Double
    ...
    Public Function ConvertdbnullsBool(ByVal x) As Boolean
    ...
    Public Function ChangeFormatDate(ByVal dtm As DateTime, ByVal format As String) As String

        Return dtm.ToString(format)

    End Function

    Public Function ConvertdbnullsDate(ByVal x) As String
    ...
    Public Function FormatPhoneNumber(ByVal x As String) As String
    ...
    Public Function SetDistinctReportName(ByRef strReportName As String) As String
    ...
    Public Function ReplaceSQLInjectionCharacters(ByVal strStringToReplace As String) As String
    ...
    Public Function CheckUserPasswordHistory(ByVal strUserID As String, ByVal strNewPassword As String) As Boolean
        ...
        While ObjDR.Read
            ...
            If CryptoUtil.EncryptTripleDes(strNewPassword) = Convertdbnulls(ObjDR("UsedPassword1")) Or CryptoUtil.EncryptTripleDes(strNewPassword) = Convertdbnulls(ObjDR("UsedPassword2")) _
                Or CryptoUtil.EncryptTripleDes(strNewPassword) = Convertdbnulls(ObjDR("UsedPassword3")) Or CryptoUtil.EncryptTripleDes(strNewPassword) = Convertdbnulls(ObjDR("UsedPassword4")) Then
    ...
    Public Sub CleanupReportDirectory()
        ...
        Dim di As New System.IO.DirectoryInfo(System.Web.HttpContext.Current.Server.MapPath(System.Configuration.ConfigurationManager.AppSettings("FilePath").ToString) & "\Downloads\Created\")
    ...
    Public Sub CleanupReportDirectory2()
        'this subrouting is called on the reports page and cleans up any files that are older than 30 minutes
        '----------------------------------------------------------------------------------------------------
        Dim configAppSettings As System.Configuration.AppSettingsReader = New System.Configuration.AppSettingsReader
        ' Create a reference to the current directory.'FilePath & "\Reports\ReportOutputFiles\"

        Dim di As New System.IO.DirectoryInfo(System.Web.HttpContext.Current.Server.MapPath(System.Configuration.ConfigurationManager.AppSettings("FilePath").ToString) & "\Reports\StartFiles\")
        ' Create an array representing the files in the current directory.
        Dim fi As System.IO.FileInfo() = di.GetFiles()
        ' Print out the names of the files in the current directory.
        Dim fiTemp As System.IO.FileInfo
    ...
    Public Shared Function GetSqlParameters(ByVal parms As System.Data.SqlClient.SqlParameterCollection, ByVal spName As String) As String
    ...
    Public Shared Function FirstDayOfQuarter(ByVal qt As Int32, ByVal FiscalYear As Int32) As String
        Select Case qt
            Case 1
                Return "10/1/" & (FiscalYear - 1)
            Case 2
    ...
    Public Shared Function LastDayOfQuarter(ByVal qt As Int32, ByVal FiscalYear As Int32) As String
        Select Case qt
            Case 1
    ...
    Public Function GetJavaScriptForNegative() As String
    ...
    Function CleanupForXML(ByVal strData As String) As String
    ...
    Public Function LineBreaksForString(ByVal strStringToSplit As String) As String
        ...
        Dim intStringLength As Integer = strTempString.Length

        Dim intLoopCount = intStringLength / 65

        Dim intLoopCounter As Integer = 0

        If intStringLength > 65 Then

            While intLoopCount < intLoopCounter
    ...
    Public Function LineBreaksForStringHTML(ByVal strStringToSplit As String, ByVal intTruncateLength As Integer) As String

        intTruncateLength = intTruncateLength

        '  Dim strTempString2 As String = "Filter out character like single quote, double quote, slash, "

        'Filter out character like single quote, double quote, slash, back slash, semi colon, extended character like NULL, carry return, new line, etc, in all strings
        Dim strTempString As String = strStringToSplit
        Dim strTempString2 As String = ""

        Dim intStringLength As Integer = strTempString.Length

        Dim intLoopCount = intStringLength / intTruncateLength

        Dim intLoopCounter As Integer = 0

        If intStringLength > intTruncateLength Then

            While intLoopCounter < intLoopCount

                If strTempString.Length > intTruncateLength Then

                    Dim strCharTruncate2 As String = GetChar(strTempString, intTruncateLength)

                    If strCharTruncate2 <> "" Then

                        Dim intTruncateLengthCount As Integer = 0

                        Dim strCharTruncate As String = GetChar(strTempString, intTruncateLength + intTruncateLengthCount)

                        While strCharTruncate <> " "

                            intTruncateLengthCount = intTruncateLengthCount + 1

                            strCharTruncate = GetChar(strTempString, intTruncateLength + intTruncateLengthCount)

                        End While

                        Dim intTempTruncateLength As Integer = intTruncateLength + intTruncateLengthCount

                        strTempString2 = strTempString2 & Mid(strTempString, 1, intTempTruncateLength) & "<br>"

                        strTempString = strTempString.Substring(intTempTruncateLength)

                    Else

                        strTempString2 = strTempString2 & Mid(strTempString, 1, intTruncateLength) & "<br>"

                        strTempString = strTempString.Substring(intTruncateLength)
            ...
            If strTempString <> "" Then
                strTempString2 = strTempString2 & strTempString
            End If

        Else

            strTempString2 = strTempString & "<br>"
