...
Partial Class NotificationPosition
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            ...
            Select Case globalMessage
                Case "1"
                    lblMessage.Text = "Position Has Been Added."
                    lblMessage.ForeColor = Drawing.Color.Green
                    lblMessage.Visible = True
                Case "2"
                    lblMessage.Text = "Position Has Been Deleted."
                    lblMessage.ForeColor = Drawing.Color.Green
                    lblMessage.Visible = True
                Case "3"
                    lblMessage.Text = "Position Has Been Updated."
                    lblMessage.ForeColor = Drawing.Color.Green
                    lblMessage.Visible = True
                Case Else
    ...
    Sub PopulateDataGrid()
        ...
        '-------------------------------------------------------------
        NotificationPositionDataGrid.CurrentPageIndex = 0
        getNotificationPosition("[NotificationPosition].[Position] ASC", "", "")
        NotificationPositionDataGrid.AllowSorting = True
    End Sub

    Sub getNotificationPosition(ByVal sSortStr As String, ByVal sSearchBy As String, ByVal sSearchText As String)
        ...
        objCmd = New SqlCommand("spFilterNotificationPosition", objConn)
    ...
    Sub SortNotificationPosition(ByVal sender As Object, ByVal e As DataGridSortCommandEventArgs)
        '-------------------------------------------------------------
        ...
        '-------------------------------------------------------------
        For x = 0 To NotificationPositionDataGrid.Columns.Count - 1
            TempHeaderHolder = NotificationPositionDataGrid.Columns(x).HeaderText
            FindImg = InStr(TempHeaderHolder, "<img")

            If FindImg <> 0 Then
                NotificationPositionDataGrid.Columns(x).HeaderText = Left(TempHeaderHolder, FindImg - 1)
            End If
        Next
        ...
        Select Case ColumnToSort.ToUpper()
            Case "POSITION"
                iIndex = 3
                NewHeaderText = "Position"
        End Select

        'Alter the column's sort expression.
        NotificationPositionDataGrid.Columns(iIndex).SortExpression = NewSortExpr

        'Alter the column's header image.
        NotificationPositionDataGrid.Columns(iIndex).HeaderText = NewHeaderText & NewHeaderImg

        'Sort the data in new order. Searches by provided.
        getNotificationPosition(NewSortExpr, ddlSearchBy.SelectedItem.Value, HelpFunction.Convertdbnulls(txtSearch.Text))
    End Sub

    Sub NotificationPositionDataGrid_PageIndexChanged(ByVal source As Object, ByVal e As DataGridPageChangedEventArgs)
        '-------------------------------------------------------------
        'This sub is called on the next and previous clicks for the
        'recordset. It cycles to the next 20 records.
        '-------------------------------------------------------------
        NotificationPositionDataGrid.CurrentPageIndex = e.NewPageIndex
        NotificationPositionDataGrid.DataBind()
        ...
        For x = 0 To NotificationPositionDataGrid.Columns.Count - 1
            ...
            FindImg = InStr(NotificationPositionDataGrid.Columns(x).HeaderText, "<img")

            If FindImg <> 0 Then
                TempSortHolder = NotificationPositionDataGrid.Columns(x).SortExpression
        ...
        If strSort = "" Then strSort = "NotificationPosition.[Position] ASC"

        getNotificationPosition(strSort, ddlSearchBy.SelectedItem.Value, HelpFunction.Convertdbnulls(txtSearch.Text))
    End Sub
    ...
    Protected Sub btnSearch_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs) Handles btnSearch.Command
        ...
        NotificationPositionDataGrid.CurrentPageIndex = 0
        getNotificationPosition("", ddlSearchBy.SelectedItem.Value, HelpFunction.Convertdbnulls(txtSearch.Text))
    End Sub

    Protected Sub NotificationPositionDataGrid_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles NotificationPositionDataGrid.ItemDataBound
    ...
    Private Sub DeleteNotificationPosition()
        Try
            Dim AuditInfo As String = ""
            Dim AuditAction As String = ""
            Dim localPosition As String = ""
            ...
            If objDR.Read() Then
                localPosition = HelpFunction.Convertdbnulls(objDR("Position"))
            End If
            ...
            AuditAction = "Deleted " & localPosition & " From Positions"

            objConn.ConnectionString = ConfigurationManager.ConnectionStrings("dbConnectionString").ConnectionString

            'Enter the email and password to query/command object.
            objCmd = New SqlCommand("spDeleteNotificationPositionByNotificationPositionID", objConn)
            objCmd.CommandType = CommandType.StoredProcedure
            objCmd.Parameters.AddWithValue("@NotificationPositionID", Request("NotificationPositionID"))
            ...
            Response.Redirect("NotificationPosition.aspx?Message=2")
        Catch ex As Exception
            DBConStringHelper.FinalizeConnection(objConn)
            lblMessage.Text = "You may not delete this Position due to the fact it is tied to related imported information. You must first delete all related imported information first, and then you may delete the Position."
            lblMessage.Visible = True
