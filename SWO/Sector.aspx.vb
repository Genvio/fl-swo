...
Partial Class Sector
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            ...
            Select Case globalMessage
                Case "1"
                    lblMessage.Text = "Sector Has Been Added."
                    lblMessage.ForeColor = Drawing.Color.Green
                    lblMessage.Visible = True
                Case "2"
                    lblMessage.Text = "Sector Has Been Deleted."
                    lblMessage.ForeColor = Drawing.Color.Green
                    lblMessage.Visible = True
                Case "3"
                    lblMessage.Text = "Sector Has Been Updated."
                    lblMessage.ForeColor = Drawing.Color.Green
                    lblMessage.Visible = True
                Case Else
    ...
    Sub PopulateDataGrid()
        ...
        '-------------------------------------------------------------
        SectorDataGrid.CurrentPageIndex = 0
        getSector("[Sector].[SectorName] ASC", "", "")
        SectorDataGrid.AllowSorting = True

    End Sub

    Sub getSector(ByVal sSortStr As String, ByVal sSearchBy As String, ByVal sSearchText As String)

        'connect and build the datagrid.
        objConn.ConnectionString = ConfigurationManager.ConnectionStrings("dbConnectionString").ConnectionString
        DBConStringHelper.PrepareConnection(objConn) 'open the connection

        objCmd = New SqlCommand("spFilterSector", objConn)
        ...
        objDataGridFunctions.CalcDataGridCounts(objDS, SectorDataGrid, "")
        'Associate the data grid with the data
        SectorDataGrid.DataSource = objDS.Tables(0).DefaultView
        SectorDataGrid.DataBind()

        objDataGridFunctions.Highlightrows(SectorDataGrid, "", "", "")

    End Sub

    Sub SortSector(ByVal sender As Object, ByVal e As DataGridSortCommandEventArgs)
        ...
        For x = 0 To SectorDataGrid.Columns.Count - 1
            TempHeaderHolder = SectorDataGrid.Columns(x).HeaderText
            FindImg = InStr(TempHeaderHolder, "<img")
            If FindImg <> 0 Then
                SectorDataGrid.Columns(x).HeaderText = Left(TempHeaderHolder, FindImg - 1)
            End If
        Next
        '--------------------------------------------------------------------------------------------------------
        ...
        SectorDataGrid.Columns(iIndex).SortExpression = NewSortExpr
        'alter the column's header image
        SectorDataGrid.Columns(iIndex).HeaderText = NewHeaderText & NewHeaderImg

        '--------------------------------------------------------------------------------------------------------
        ' Sort the data in new order
        'searches by provided
        getSector(NewSortExpr, ddlSearchBy.SelectedItem.Value, HelpFunction.Convertdbnulls(txtSearch.Text))

    End Sub

    Sub SectorDataGrid_PageIndexChanged(ByVal source As Object, ByVal e As DataGridPageChangedEventArgs)

        '---------------------------------------------------------------------------------------------------------
        '   This sub is called on the next and previous clicks for the recordset, it cycles to the next 20 records
        '---------------------------------------------------------------------------------------------------------
        SectorDataGrid.CurrentPageIndex = e.NewPageIndex
        SectorDataGrid.DataBind()
        ...
        For x = 0 To SectorDataGrid.Columns.Count - 1
            FindImg = InStr(SectorDataGrid.Columns(x).HeaderText, "<img") 'find the column with the <img tag
            If FindImg <> 0 Then
                TempSortHolder = SectorDataGrid.Columns(x).SortExpression
        ...
        If strSort = "" Then strSort = "Sector.[SectorID] ASC"
        getSector("", ddlSearchBy.SelectedItem.Value, HelpFunction.Convertdbnulls(txtSearch.Text))
    End Sub
    ...
    Private Sub DeleteSector()
        Try
            ...
            AuditAction = "Deleted """ & localSectorName & """ from Sectors"
            objConn.ConnectionString = ConfigurationManager.ConnectionStrings("dbConnectionString").ConnectionString

            '// Enter the email and password to query/command object.
            objCmd = New SqlCommand("spDeleteSectorBySectorID", objConn)
            objCmd.CommandType = CommandType.StoredProcedure
            objCmd.Parameters.AddWithValue("@SectorID", Request("SectorID"))
            ...
            Response.Redirect("Sector.aspx?Message=2")

        Catch ex As Exception
            DBConStringHelper.FinalizeConnection(objConn)
            lblMessage.Text = "Error deleting Sector.<br />(A sector may not be deleted if it is associated with one or more incidents or positions.)"
            lblMessage.Visible = True
