...
Partial Class RegionCoordinator
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            ...
            Select Case globalMessage
                Case "1"
                    lblMessage.Text = "Region Coordinator Has Been Added."
                    lblMessage.ForeColor = Drawing.Color.Green
                    lblMessage.Visible = True
                Case "2"
                    lblMessage.Text = "Region Coordinator Has Been Deleted."
                    lblMessage.ForeColor = Drawing.Color.Green
                    lblMessage.Visible = True
                Case "3"
                    lblMessage.Text = "Region Coordinator Has Been Updated."
                    lblMessage.ForeColor = Drawing.Color.Green
                    lblMessage.Visible = True
                Case Else
    ...
    Sub PopulateDataGrid()
        ...
        '-------------------------------------------------------------
        RegionCoordinatorDataGrid.CurrentPageIndex = 0
        getRegionCoordinator("[RegionCoordinator].[RegionCoordinatorName] ASC", "", "")
        RegionCoordinatorDataGrid.AllowSorting = True

    End Sub

    Sub getRegionCoordinator(ByVal sSortStr As String, ByVal sSearchBy As String, ByVal sSearchText As String)

        'connect and build the datagrid.
        objConn.ConnectionString = ConfigurationManager.ConnectionStrings("dbConnectionString").ConnectionString
        DBConStringHelper.PrepareConnection(objConn) 'open the connection

        objCmd = New SqlCommand("spFilterRegionCoordinator", objConn)
        ...
        objDataGridFunctions.CalcDataGridCounts(objDS, RegionCoordinatorDataGrid, "")
        'Associate the data grid with the data
        RegionCoordinatorDataGrid.DataSource = objDS.Tables(0).DefaultView
        RegionCoordinatorDataGrid.DataBind()

        objDataGridFunctions.Highlightrows(RegionCoordinatorDataGrid, "", "", "")

    End Sub

    Sub SortRegionCoordinator(ByVal sender As Object, ByVal e As DataGridSortCommandEventArgs)
        ...
        For x = 0 To RegionCoordinatorDataGrid.Columns.Count - 1
            TempHeaderHolder = RegionCoordinatorDataGrid.Columns(x).HeaderText
            FindImg = InStr(TempHeaderHolder, "<img")
            If FindImg <> 0 Then
                RegionCoordinatorDataGrid.Columns(x).HeaderText = Left(TempHeaderHolder, FindImg - 1)
            End If
        Next
        '--------------------------------------------------------------------------------------------------------
        ...
        Select Case ColumnToSort.ToUpper()
            ...
            Case "REGIONCOORDINATORNAME"
                iIndex = 3
                NewHeaderText = "Name"
            Case "REGION"
                iIndex = 4
                NewHeaderText = "Region"

        End Select
        '--------------------------------------------------------------------------------------------------------
        ' alter the column's sort expression
        RegionCoordinatorDataGrid.Columns(iIndex).SortExpression = NewSortExpr
        'alter the column's header image
        RegionCoordinatorDataGrid.Columns(iIndex).HeaderText = NewHeaderText & NewHeaderImg

        '--------------------------------------------------------------------------------------------------------
        ' Sort the data in new order
        'searches by provided
        getRegionCoordinator(NewSortExpr, ddlSearchBy.SelectedItem.Value, HelpFunction.Convertdbnulls(txtSearch.Text))

    End Sub

    Sub RegionCoordinatorDataGrid_PageIndexChanged(ByVal source As Object, ByVal e As DataGridPageChangedEventArgs)

        '---------------------------------------------------------------------------------------------------------
        '   This sub is called on the next and previous clicks for the recordset, it cycles to the next 20 records
        '---------------------------------------------------------------------------------------------------------
        RegionCoordinatorDataGrid.CurrentPageIndex = e.NewPageIndex
        RegionCoordinatorDataGrid.DataBind()
        ...
        For x = 0 To RegionCoordinatorDataGrid.Columns.Count - 1
            FindImg = InStr(RegionCoordinatorDataGrid.Columns(x).HeaderText, "<img") 'find the column with the <img tag
            If FindImg <> 0 Then
                TempSortHolder = RegionCoordinatorDataGrid.Columns(x).SortExpression
        ...
        If strSort = "" Then strSort = "RegionCoordinator.[RegionCoordinatorID] ASC"


        getRegionCoordinator("", ddlSearchBy.SelectedItem.Value, HelpFunction.Convertdbnulls(txtSearch.Text))
    ...
    Protected Sub RegionCoordinatorDataGrid_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles RegionCoordinatorDataGrid.ItemDataBound
    ...
    Private Sub DeleteRegionCoordinator()
        ...
        Try
            ...
            AuditAction = "Deleted " & localRegionCoordinatorName & " from Associated Tasks"

            objConn.ConnectionString = ConfigurationManager.ConnectionStrings("dbConnectionString").ConnectionString

            '// Enter the email and password to query/command object.
            objCmd = New SqlCommand("spDeleteRegionCoordinatorByRegionCoordinatorID", objConn)
            objCmd.CommandType = CommandType.StoredProcedure
            objCmd.Parameters.AddWithValue("@RegionCoordinatorID", Request("RegionCoordinatorID"))
            ...
            Response.Redirect("RegionCoordinator.aspx?Message=2")

        Catch ex As Exception

            DBConStringHelper.FinalizeConnection(objConn)
            lblMessage.Text = "You may not delete this Region Coordinator due to the fact it is tied to related imported information. You must first delete all related imported information first, and then you may delete the Region Coordinator."
            lblMessage.Visible = True
